...
class Yolov1(nn.Module):
  def __init__(self, in_channels=3, **kwargs):
    ...
    self.darknet = self._conv_layers(self.architecture)
    self.fc = self._fc_layers(**kwargs)
  ...
  def _fc_layers(self, split_size, num_bboxes, num_classes):
    S, B, C = split_size, num_bboxes, num_classes
    in_size = 1024 * S * S
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(in_size, 496),
        nn.Dropout(0.5),
        nn.LeakyReLU(0.1),
